        ...
        <div className="p-4 border-b border-gray-200 flex flex-col sm:flex-row items-center justify-between gap-4">
            <div className="relative w-full sm:w-auto sm:max-w-xs">
                ...
                <input
                    ...
                    className="block w-full pl-10 pr-3 py-2 border border-gray-300 rounded-lg leading-5 bg-white placeholder-gray-500 focus:outline-none focus:ring-1 focus:ring-primary-main focus:border-primary-main sm:text-sm"
                />
            </div>
            ...
            <div className="flex items-center gap-2">
                {selectedRowCount > 0 && (
                    <div className="relative" ref={bulkMenuRef}>
                        <button 
                            ...
                            className="flex items-center gap-2 px-4 py-2 border border-primary-main text-primary-main rounded-lg text-sm font-medium hover:bg-primary-lighter"
                        >
                        ...
                        {isBulkMenuOpen && (
                            <div className="absolute right-0 mt-2 w-48 bg-white rounded-md shadow-lg z-10 border border-gray-200">
                                <ul className="py-1 text-sm text-gray-700">
                                    <li><button onClick={() => { onBulkAction('Generate Manifest'); setBulkMenuOpen(false); }} className="w-full text-left px-4 py-2 hover:bg-gray-100">Generate Manifest</button></li>
                                    <li><button onClick={() => { onBulkAction('Generate Labels'); setBulkMenuOpen(false); }} className="w-full text-left px-4 py-2 hover:bg-gray-100">Generate Labels</button></li>
                                    <li><button onClick={() => { onBulkAction('Cancel Orders'); setBulkMenuOpen(false); }} className="w-full text-left px-4 py-2 text-error-main hover:bg-gray-100">Cancel Orders</button></li>
                                </ul>
                            </div>
                        )}
                    </div>
                ...
                <button className="flex items-center gap-2 px-4 py-2 border border-gray-300 rounded-lg text-sm font-medium hover:bg-gray-100">
                    <FilterIcon className="text-gray-600"/> Filters
                </button>
                ...
                <div className="relative" ref={columnMenuRef}>
                    ...
                    >
                        <ViewColumnsIcon className="text-gray-600"/> Columns
                    </button>
                     {isColumnMenuOpen && (
                        <div className="absolute right-0 mt-2 w-56 bg-white rounded-md shadow-lg z-10 border border-gray-200">
                            <div className="p-2 text-sm font-semibold text-gray-800 border-b">Toggle Columns</div>
                            <ul className="py-2 px-2 text-sm text-gray-700 grid grid-cols-1 gap-1">
                                {toggleableColumns.map(col => (
                                    <li key={col.id}>
                                        <label className="flex items-center gap-2 px-2 py-1.5 hover:bg-gray-100 rounded-md cursor-pointer">
                                            <input 
                                                type="checkbox" 
