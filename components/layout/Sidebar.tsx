...
const Sidebar: React.FC<SidebarProps> = ({ navConfig, isMobileOpen, onCloseMobile, isCollapsed }) => {
  ...
  return (
    <>
      ...
      <aside className={`fixed top-0 left-0 h-full bg-gray-800 text-white transform ${isMobileOpen ? 'translate-x-0' : '-translate-x-full'} lg:relative lg:translate-x-0 transition-all duration-300 ease-in-out z-30 flex flex-col ${sidebarWidth}`}>
        ...
        <div className="p-4 border-t border-gray-700 hidden lg:block">
            <Link to="/settings" className="flex items-center w-full p-2 text-gray-400 rounded-md hover:bg-gray-700 hover:text-white">
...
const NavItemComponent: React.FC<{ item: NavItem, isCollapsed: boolean }> = ({ item, isCollapsed }) => {
        ...
        return (
            <div>
                <button
                    ...
                    className="w-full flex items-center justify-between p-2 text-left text-gray-300 rounded-md hover:bg-gray-700 hover:text-white focus:outline-none"
                >
    ...
    return (
        <NavLink
            to={item.path!}
            className={({ isActive }) => 
                `flex items-center p-2 rounded-md text-gray-300 hover:bg-gray-700 hover:text-white ${isActive ? 'bg-gray-900 text-white' : ''} ${isCollapsed ? 'justify-center' : ''}`
            }
